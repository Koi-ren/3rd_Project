...
def info():
    ...
    try:
        ...
        if ground:
            ground.sync_with_info(formatted_data)  # 예측 상태 동기화
        logging.info(f"Stored /info data: time={formatted_data['time']}, playerPos={formatted_data['playerPos']}, body_angle={formatted_data['playerBodyX']}")
        print(f"Stored /info data: time={formatted_data['time']}, playerPos={formatted_data['playerPos']}, body_angle={formatted_data['playerBodyX']}")
...
def get_move():
    ...
    weight = min(max(round(command.get("weight", 1.0), 2), 0.1), 1.0)
    return jsonify({"move": move, "weight": weight}), 200
