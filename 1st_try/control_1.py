...
class GameServer:
    def __init__(self):
        ...
        self.character = Kinematic(position=Vector(5.935, 25.696), orientation=0.0)
        self.target = Kinematic(position=Vector(13.546, 27.687), orientation=0.0)
        self.arrive = Arrive(
            character=self.character,
            target=self.target,
            maxAcceleration=0.5,      # coord/s²
            maxSpeed=1.944,          # 19.44 m/s ÷ 10
            targetRadius=1.,        # 1m ÷ 10
            slowRadius=1.,          # 10m ÷ 10
            timeToTarget=0.1
        )
        self.time_step = 0.125  # 평균 초당 8회
        self.input_count_w = 0
        self.input_count_a = 0  # A: 양수, D: 음수
        self.current_speed = 0.0
        self.max_rotation_per_step = math.radians(5.0)  # 최대 5도/프레임
        self.last_command = "STOP"
        self.shared_key_value.set_key_value("STOP")
        self.last_position = Vector(5.935, 25.696)
        self.map_bounds = (0, 30, 0, 30)  # x: 0~300m, z: 0~300m (coord 단위)

    def calculate_speed(self, input_count):
        """속도 계산 (player_speed와 동기화)"""
        if input_count <= 0:
            return 0.0
        # 회귀식 기반 속도, 최대값 제한
